...
MenuEX::MenuEX(s32 X, s32 Y, s32 Width, s32 Height, draw::Color OptionColor, s32 ItemSize, s32 SelectSize, s32 ItemsToShow) : Element::Element()
{
	this->h = Height;
	this->pretouch = false;
	this->uptime = false;
...
void MenuEX::SetHeight(s32 Height)
{
	this->h = Height;
}
...
draw::Color MenuEX::GetColor()
{
	return this->clr;
}

void MenuEX::SetColor(draw::Color Color)
{
	this->clr = Color;
}

draw::Color MenuEX::GetOnFocusColor()
{
	return this->fcs;
}

void MenuEX::SetOnFocusColor(draw::Color Color)
{
	this->fcs = Color;
}

draw::Color MenuEX::GetScrollbarColor()
{
	return this->scb;
}

void MenuEX::SetScrollbarColor(draw::Color Color)
{
	this->scb = Color;
}
...
void MenuEX::AddItem(MenuItem *Item)
{
...
MenuItem *MenuEX::GetSelectedItem()
{
...
void MenuEX::OnRender(render::Renderer *Drawer)
{
	...
	if (!this->itms.empty())
	{
		...
		for (u32 ii = 0; ii < this->ishow; ii++)
		{
			...
			u32 xw = render::GetTextWidth(itm->GetFont(), itm->GetName());
			...
			if (this->isel == i)
			{
				Drawer->RenderRectangleEx(draw::Color(5, 185, 140, this->alpha_count), cx, cy, cw, ch, 4);
...
void MenuEX::OnInputTouch(u64 Down, u64 Up, u64 Held, bool Touch, bool Focus)
{
	touchPosition tch;
	hidTouchRead(&tch, 0);
...
void MenuEX::OnInput(u64 Down, u64 Up, u64 Held, bool Touch, bool Focus)
{
	if (this->itms.empty())
		return;
	...
	else
	{
		...
		{
			OnInputTouch(Down, Up, Held, Touch, Focus);
		}
